...
public class RippleItemAnimator extends SimpleItemAnimator {
    ...
    List<RecyclerView.ViewHolder> pendingAdd = new ArrayList<RecyclerView.ViewHolder>();
    List<RecyclerView.ViewHolder> pendingRemove = new ArrayList<RecyclerView.ViewHolder>();

    @Override
    public void runPendingAnimations() {
        int animationDuration = 300;
        if (!pendingAdd.isEmpty()) {
            for (final RecyclerView.ViewHolder viewHolder : pendingAdd) {
        ...
        if (!pendingRemove.isEmpty()) {
            for (final RecyclerView.ViewHolder viewHolder : pendingRemove) {
                View target = viewHolder.itemView;
                target.setPivotX(target.getMeasuredWidth() / 2);
                target.setPivotY(target.getMeasuredHeight() / 2);
    ...
    public boolean animateAdd(RecyclerView.ViewHolder viewHolder) {
        ...
        return pendingAdd.add(viewHolder);
    }
    ...
    public boolean isRunning() {
        return !pendingAdd.isEmpty() || !pendingRemove.isEmpty();
    }
}